...
async function ensureWakeMainWorld(sender = {}) {
    ...
    await chrome.scripting.executeScript({
        target: { tabId },
        world: 'MAIN',
        func: (wakeWordUrl, wakeBridgeUrl) => {
            ...
            const existingWakeWord = root.querySelector('script[data-hades-wake-word="true"]');
            const existingBridge = root.querySelector('script[data-hades-wake-bridge="true"]');
            const wakeWordReady = existingWakeWord && existingWakeWord.getAttribute('src') === wakeWordUrl;
            const bridgeReady = existingBridge && existingBridge.getAttribute('src') === wakeBridgeUrl;

            if (wakeWordReady && bridgeReady) {
                emitReady();
                return;
            }
            ...
            if (existingWakeWord && !wakeWordReady) {
                existingWakeWord.remove();
            }

            let wakeWordScript = root.querySelector('script[data-hades-wake-word="true"]');
            if (!wakeWordScript) {
                wakeWordScript = document.createElement('script');
                wakeWordScript.dataset.hadesWakeWord = 'true';
                wakeWordScript.src = wakeWordUrl;
                wakeWordScript.async = false;
                root.appendChild(wakeWordScript);
            }

            const bridgeScript = document.createElement('script');
            bridgeScript.dataset.hadesWakeBridge = 'true';
            bridgeScript.src = wakeBridgeUrl;
            bridgeScript.async = false;
            root.appendChild(bridgeScript);
        },
        args: [
            chrome.runtime.getURL('wake-word-shared.js'),
            chrome.runtime.getURL('wake-bridge.js')
        ]
    });
...
async function claimVoiceOwner(message = {}, sender = {}) {
    ...
    const canTakeOwnership = current
        ? (isSameVoiceOwner(current, sender, instanceId) || visible)
        : visible;
    ...
    return {
        ok: true,
        owner: Boolean(next && next.instanceId === instanceId),
        state: next || null
    };
}
...
async function postOpsEvent(type, payload = {}) {
    if (!type) return;
    try {
        await requestBackend('/ops/event', {
            method: 'POST',
            body: {
                type,
                payload
        ...
        // Ops timeline kritik yol degil; sessizce devam et.
    }
}

async function dispatchDesktopAction(action = '', detail = {}) {
    const safeAction = String(action || '').trim();
    if (!safeAction) {
        return { ok: false, error: 'Desktop action bos olamaz.' };
    }

    const tabs = await chrome.tabs.query({ url: CHATGPT_URL_PATTERNS });
    if (!tabs.length) {
        return { ok: false, error: 'Acik HADES/ChatGPT sekmesi bulunamadi.' };
    }
    ...
    for (const tab of orderedTabs) {
        ...
        try {
            ...
            lastError = new Error(response?.error || 'Desktop action reddedildi.');
        } catch (error) {
            lastError = error;
        }
    }
    ...
    return {
        ok: false,
        error: lastError?.message || 'Desktop action gonderilemedi.'
    };
}

function isDirectBrowserUrlLike(value = '') {
...
function buildBrowserOpenPayload(args = {}) {
    const rawTarget = String(args.url || args.query || '').trim();
    if (!rawTarget) {
        return {
            url: '',
            query: '',
    ...
    if (isDirectBrowserUrlLike(rawTarget)) {
        return {
            url: rawTarget,
            query: '',
            reason: 'tool_open'
...
async function executeActions(actions = []) {
    ...
    for (const action of Array.isArray(actions) ? actions : []) {
        ...
        await postOpsEvent('tool.dispatch', {
            tool,
            args
        });
        const result = await executeSingleAction(action);
        results.push(result);
        await postOpsEvent('tool.result', {
            tool,
            args,
...
async function executeSingleAction(rawAction = {}) {
    ...
    try {
        switch (tool) {
            ...
            case 'browser.search':
                return normalizeBackendResult(tool, await requestBackend('/ops/browser/open', {
                    method: 'POST',
                    body: {
                        query: args.query,
                        source: 'bridge_tool',
            ...
            case 'browser.open':
                {
                    const browserTarget = buildBrowserOpenPayload(args);
                    return normalizeBackendResult(tool, await requestBackend('/ops/browser/open', {
                        method: 'POST',
                        body: {
                            url: browserTarget.url,
                            query: browserTarget.query,
                            source: 'bridge_tool',
                            reason: browserTarget.reason
                        }
                    }));
                }
            case 'browser.panel': {
                ...
                if (action === 'status') {
                    return normalizeBackendResult(tool, await requestBackend('/ops/ui-state'));
                }
                if (action === 'show') {
                    await dispatchDesktopAction('open-ops-cockpit');
                }
                return normalizeBackendResult(tool, await requestBackend('/ops/ui-state', {
                    method: 'POST',
                    body: {
                        browserPanelVisible: action === 'show'
                    }
                }));
            }
            case 'cockpit.window': {
                const action = String(args.action || 'status').trim().toLowerCase();
                if (action === 'status') {
                    const result = await dispatchDesktopAction('ops-cockpit-status');
                    return {
                        tool,
                        ok: Boolean(result.ok),
                        data: result.ok ? result.response : null,
                        error: result.ok ? null : result.error
                    };
                }
                const result = await dispatchDesktopAction(action === 'minimize' ? 'minimize-ops-cockpit' : 'open-ops-cockpit');
                return {
                    tool,
                    ok: Boolean(result.ok),
                    data: result.ok ? result.response : null,
